...
from datasets.ycb.ycb_dataset import Dataset as Dataset
...
def cal_view_pred_pose(model, data, epoch=0, obj_id=-1):
    ...
    with torch.set_grad_enabled(False):
        ...
        vis_dir = os.path.join(config.log_eval_dir, args.folder)
        ensure_fd(vis_dir)
...
def main():
    ...
    for i, data in tqdm.tqdm(
        enumerate(test_loader), total=len(test_loader), desc="val"
    ):
